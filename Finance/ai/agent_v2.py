from langchain_core.messages import HumanMessage, SystemMessage, ToolMessage
from .llm import get_llm
from .tools import (
    get_financial_summary_tool,
    get_total_expense_tool
)
from .prompts import FINANCE_AGENT_SYSTEM_PROMPT
from langchain.tools import tool

# Existing memory
from .memory import (
    load_messages,
    save_messages,
    build_system_prompt,
    update_summary,
    update_long_term_memory,
    detect_important_info,
    save_meta,
    load_meta
)

# 🔥 NEW: vector memory
from .memory_vector import add_memory, get_relevant_memory


# ----------------------------
# TOOL CONTEXT
# ----------------------------
def get_tools_with_context(building_id):
    
    @tool
    def get_total_expense_tool_ctx(year: int) -> int:
        """
    Get financial summary for a building for a financial year.
    """
        return get_total_expense_tool.invoke({
            "year": year,
            "building_id": building_id
        })

    @tool
    def get_financial_summary_tool_ctx(year: int):
        """
    Get total expense for a financial year for a specific building.

    Args:
        year (int): Financial year start (e.g., 2024 means Apr 2024–Mar 2025)
        

    Returns:
        int: Total expense amount
    """
        return get_financial_summary_tool.invoke({
            "year": year,
            "building_id": building_id
        })

    return [
        get_total_expense_tool_ctx,
        get_financial_summary_tool_ctx
    ]

# ----------------------------
# RESPONSE CLEANER
# ----------------------------
def extract_text_from_response(content):
    if isinstance(content, str):
        return content
    
    if isinstance(content, list):
        texts = []
        for item in content:
            if isinstance(item, dict) and item.get("type") == "text":
                texts.append(item.get("text", ""))
        return "\n".join(texts)
    
    return str(content)


# ----------------------------
# 🔥 FILTER WHAT TO STORE
# ----------------------------
def should_store(text):
    text = text.lower()

    # store if meaningful length
    if len(text.split()) > 6:
        return True

    # or important keywords
    keywords = [
        "expense", "summary", "payment",
        "plan", "idea", "strategy",
        "problem", "solution", "decision",
        "project", "goal"
    ]

    return any(k in text for k in keywords)


# ----------------------------
# MAIN AGENT (V2)
# ----------------------------
def run_finance_agent(user_id, session_id, user_query, building):
    llm = get_llm()

    # Tools
    tools = get_tools_with_context(building.id)
    llm_with_tools = llm.bind_tools(tools)

    # ----------------------------
    # 🧠 LOAD MEMORY
    # ----------------------------
    base_messages = load_messages(user_id, session_id, FINANCE_AGENT_SYSTEM_PROMPT)

    system_prompt = build_system_prompt(
        user_id,
        session_id,
        FINANCE_AGENT_SYSTEM_PROMPT
    )

    # ----------------------------
    # 🔥 RETRIEVAL (VECTOR MEMORY)
    # ----------------------------
    try:
        vector_memories = get_relevant_memory(user_id, user_query, k=3)
    except Exception as e:
        print("Vector retrieval error:", e)
        vector_memories = []

    # ----------------------------
    # 🧠 BUILD CONTROLLED PROMPT
    # ----------------------------
    messages = [SystemMessage(content=system_prompt)]

    # 🔥 Inject relevant memory as context
    if vector_memories:
        messages.append(
            SystemMessage(
                content="Relevant past context:\n" + "\n".join(vector_memories)
            )
        )

    # 🔥 Recency window (STRICT)
    messages += base_messages[-2:]

    # Current user query
    messages.append(HumanMessage(content=user_query))

    # ----------------------------
    # 🔁 TOOL LOOP
    # ----------------------------
    while True:
        response = llm_with_tools.invoke(messages)
        messages.append(response)

        if response.tool_calls:
            for tool_call in response.tool_calls:
                tool_name = tool_call["name"]
                tool_args = tool_call["args"]

                tool_dict = {t.name: t for t in tools}
                tool_func = tool_dict[tool_name]

                result = tool_func.invoke(tool_args)

                messages.append(
                    ToolMessage(
                        content=str(result),
                        tool_call_id=tool_call["id"]
                    )
                )
        else:
            # ----------------------------
            # ✅ FINAL RESPONSE
            # ----------------------------
            response_text = extract_text_from_response(response.content)

            # ----------------------------
            # 💾 SHORT-TERM MEMORY (LIMITED)
            # ----------------------------
            cleaned_messages = (
                base_messages +
                [HumanMessage(content=user_query), response]
            )[-6:]

            save_messages(user_id, session_id, cleaned_messages)

            # ----------------------------
            # 🔥 VECTOR MEMORY STORAGE (FIXED)
            # ----------------------------
            try:
                # ✅ ALWAYS store user input
                add_memory(user_id, f"User: {user_query}")
                print("📦 Stored USER:", user_query[:50])

                # ✅ Store AI only if useful
                if should_store(response_text):
                    add_memory(user_id, f"Assistant: {response_text}")
                    print("📦 Stored AI:", response_text[:50])

            except Exception as e:
                print("❌ Vector store error:", e)

            # ----------------------------
            # 🧠 META MEMORY (FIXED)
            # ----------------------------
            meta = load_meta(user_id, session_id)

            # 🔥 GLOBAL COUNTER (IMPORTANT)
            meta["total_messages"] = meta.get("total_messages", 0) + 1

            # ----------------------------
            # 📄 SUMMARY UPDATE (COMPRESSION)
            # ----------------------------
            if meta["total_messages"] - meta.get("last_summary_update", 0) >= 5:
                try:
                    update_summary(llm, user_id, session_id, cleaned_messages)
                    meta["last_summary_update"] = meta["total_messages"]
                    print("✅ Summary updated")
                except Exception as e:
                    print("❌ Summary update failed:", e)

            # ----------------------------
            # 🧠 LONG-TERM MEMORY UPDATE
            # ----------------------------
            if meta["total_messages"] - meta.get("last_memory_update", 0) >= 7:
                try:
                    update_long_term_memory(llm, user_id, cleaned_messages)
                    meta["last_memory_update"] = meta["total_messages"]
                    print("✅ Long-term memory updated")
                except Exception as e:
                    print("❌ Long-term memory failed:", e)

            # ----------------------------
            # ⚡ EVENT-BASED MEMORY
            # ----------------------------
            if detect_important_info(user_query):
                try:
                    update_long_term_memory(llm, user_id, cleaned_messages)
                    meta["last_memory_update"] = meta["total_messages"]
                    print("⚡ Event memory updated")
                except Exception as e:
                    print("❌ Event memory failed:", e)

            # ----------------------------
            # 💾 SAVE META
            # ----------------------------
            save_meta(user_id, session_id, meta)

            return response_text
